...
int enqueue(int* cq, int n, int front, int rear)
{
    if((rear + 1) % n == front) // --3 // Due to this condition (to distinguish between full and empty queue) and because at the initial condition(when front = rear), we insert the first element at rear + 1, so, the rear value(which is = front before doing rear = rear + 1) remains empty so we have done n++, if we don't do n++ above, then we are only able to enqueue n-1 elements. Therefore to enqueue n elements first we do n++.
