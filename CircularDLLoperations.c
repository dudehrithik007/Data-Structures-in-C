...
void main()
{
    printf("\n\nTo implement various operations on a Circular-Doubly-Linked-List in a menu-driven program.");
...
int traversal(int flag) // This function is used to traverse through the wbole list. The argument flag is used to make two versions of the same function in one function itself. When flag is 0, we print each node's value and count the total number of nodes while while traversing the list. Whwn flag is 1, we only count the total number of nodes in the list. In both the cases, we return the total number of nodes present in the list. According to the flag value passed at the time of calling, the function will behave accordingly as explained.
{
    ...
    else // When the list contains at least 1 element.
    {
        ...
        while((trav -> next) != head) // This is used to count as well as print the values after the first node's value in the list. // Only the while loop condition is different here from the SLL ((trav -> next) != NULL). Same as Circular SLL.
        {
            trav = trav -> next;
...
void insertion() // Inserts a node at the position specified by the user in the list.
{
    ...
    if(head == NULL) // or tail == NULL // When the list is empty. // Making the new node 'newNode' as the first node. // Here we need to change head's as well as tail's value.
    {
        ...
        newNode -> prev = newNode; // To maintain the circular property.
    }
    else // When the list contains at least 1 node.
    {
        ...
        if(pos == 1) // Inserting a node at the start of the list (When the list already contains at least one node). // Here we need to change head's as well as tail's value to maintain the circular property.
        {
            newNode -> next = head;
            head -> prev = newNode;
            head = newNode;
            head -> prev = tail; // To maintain the circular property.
        ...
        else // Inserting a node at any position other than the start of the list (When the list already contains at least one node).
        {
            ...
            newNode -> next = trav -> next; // To maintain the circular property if pos = max, else, it also works to maintain the links correctly.
            (newNode -> next) -> prev = newNode; // To maintain the circular property if pos = max, else, it also works to maintain the links correctly.
            newNode -> prev = trav;
            trav -> next = newNode;
            if(pos == max) // This is a new case which was not in SLL as we have tail which points to the last node of the list in Circular DLL. So, only changing tail when inserting a node at the last/end of the list.
                tail = newNode;
        }
    }
...
void deletion() // Deletes a node from the position specified by the user from the list.
{
    ...
    else // When the list contains at least 1 node.
    {
        ...
        else // When the list has at least 2 nodes. // We make separate cases till 2 nodes as we have 2 pointers to keep track of the list here. In SLL we created separate cases till 1 node as there we have only one pointer 'head' to keep track of the list.
        {
            ...
            if(pos == 1) // Deleting the first node. // Here we need to change head's and tail's value as well.
            {
                temp = head;
                head = head -> next;
                head -> prev = tail; // To maintain the circular property.
            ...
            else // Deleting a node other than the first node. // Here we need to change tail's value only if we are deleting the last node.
            {
                ...
                temp = trav -> next;
                trav ->  next = temp -> next; // To maintain the circular property if pos = max, else, it also works to maintain the links correctly. // Also makes possible here itself the case of deletion of the last node where the second last node which becomes the last node (after deletion of the current last node) points to the first node.
                (temp -> next) -> prev = trav; // To maintain the circular property if pos = max, else, it also works to maintain the links correctly. // Also makes possible here itself the case of deletion of the last node where head node (after deletion of the current last node) points to the new last node.
...
void search() // Searching for all instances of a data value entered by the user.
{
    ...
    for(int i = 1; i <= traversal(1); i++) // Traversing the nodes till the current size of the list. // i is used to dispay the positions of the value if found in the list. // i = 1 because initially we(trav) point(s) to head above the for loop, that is, we are at the position 1 in the list. Even if the list is empty we are not using the value of i later in the code after the loop as we won't enter the loop (i's value is only used inside the loop and we enter the loop if the list contains at least 1 node) in that case, so, setting i to 1 makes no difference in that case. Otherwise, if the list contains at least 1 node, initially, i = 1 means we point to position 1 in the list that is at the first node which is also pointed to by trav.
    {
        ...
        trav = trav -> next;
    } // After exiting the loop i contains the value = no. of elements in the list + 1 and trav contains head. As we don't use i and trav outside the loop, so it doesn't matter.
    if(flag == 0)
        printf("\nData not found.\n");
